...
def sample_to_sample_plus(samples, cfg, datamode):
    ...
    for sample in samples:
        ...
        new_samples += [
            SamplePlus(x.cpu(), y.cpu(), y_outer.cpu(), shape=shape)
        ]
...
def voxel2mesh(volume, gap, shape):
    ...
    vertices_mc, faces_mc, _, _ = measure.marching_cubes_lewiner(
        volume.cpu().data.numpy(), 0, step_size=gap, allow_degenerate=False)
    vertices_mc = torch.flip(torch.from_numpy(vertices_mc),
