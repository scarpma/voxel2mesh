...
def uniform_unpool(vertices_, faces_, identical_face_batch=True):
    ...
    for vertices, faces in zip(vertices_, faces_):
        ...
        unique_edges, unique_edge_indices = torch.unique(unique_edges,
                                                         return_inverse=True,
                                                         dim=0)
        ...
        new_vertices = torch.cat(
            ...
            dim=0)  # <----------------------- new vertices + old vertices
        ...
        for j, combination in enumerate(edge_combinations_3):
            ...
            new_vertex_1 = unique_edge_indices[torch.arange(
                0, 3 * face_count, 3) + combination[0]] + vertices_count
            new_vertex_2 = unique_edge_indices[torch.arange(
                0, 3 * face_count, 3) + combination[1]] + vertices_count

            middle_face += [new_vertex_1[:, None], new_vertex_2[:, None]]
            corner_faces += [
                torch.cat([
                    common_vertex[:, None], new_vertex_1[:, None],
                    new_vertex_2[:, None]
                ],
                          dim=1)
            ]
        ...
        new_faces_all += [torch.cat([corner_faces, middle_face],
                                    dim=0)[None]]  # new faces-3
...
def adoptive_unpool(vertices, faces_prev, sphere_vertices, latent_features,
                    N_prev):
    ...
    if latent_features is not None:
        latent_features = torch.cat(
            [latent_features_primary, latent_features_needed], dim=0)[None]

    sphere_vertices = torch.cat(
        [sphere_vertices_primary, sphere_vertices_needed], dim=0)
    sphere_vertices = sphere_vertices / torch.sqrt(
        torch.sum(sphere_vertices**2, dim=1)[:, None])
