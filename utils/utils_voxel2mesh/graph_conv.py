...
class GraphConvEdgeLengthWeighted(nn.Module):
    ...
    def forward(self, input, A, Dinv, vertices, faces):
        ...
        all_edges = faces[:, :, edges].long()
        all_edges = all_edges.view(1, -1, 2)
        dist[0, all_edges[0, :, 0], all_edges[0, :, 1]] = torch.exp(-torch.sum(
            (vertices[0, all_edges[0, :, 0]] - vertices[0, all_edges[0, :, 1]])
            **2,
            dim=1) / (self.sigma**2))
...
class Feature2VertexLayer(nn.Module):
    def __init__(self, in_features, hidden_layer_count, batch_norm=False):
        ...
        for i in range(hidden_layer_count, 1, -1):
            self.gconv += [
                GraphConv(i * in_features // hidden_layer_count,
    ...
    def forward(self, features, adjacency_matrix, degree_matrix, vertices,
                faces):
        for gconv_hidden in self.gconv:
            features = F.relu(
                gconv_hidden(features, adjacency_matrix, degree_matrix,
                             vertices, faces))
...
class Features2Features(nn.Module):
    ...
    def forward(self, features, adjacency_matrix, degree_matrix, vertices,
                faces):
        features = F.relu(
            self.gconv_first(features, adjacency_matrix, degree_matrix,
                             vertices, faces))
