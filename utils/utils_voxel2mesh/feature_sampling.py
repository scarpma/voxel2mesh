...
class SkipConnections(nn.Module):
    def __init__(self, config, features_count):
        ...
        self.shift = torch.tensor(list(product(
            (-1, 0, 1), repeat=3)))[None].float() * torch.tensor(
                [[[2 / (W - 1), 2 / (H - 1), 2 / (D - 1)]]])[None]
        self.shift = self.shift.cuda()

        self.sum_neighbourhood = nn.Conv2d(features_count,
                                           features_count,
                                           kernel_size=(1, 27),
                                           padding=0).cuda()

        torch.nn.init.kaiming_normal_(self.sum_neighbourhood.weight,
                                      nonlinearity='relu')
    ...
    def forward(self, voxel_features, vertices):
        ...
        features = F.grid_sample(voxel_features,
                                 neighbourhood,
                                 mode='bilinear',
                                 padding_mode='border',
                                 align_corners=True)
...
class NeighbourhoodSampling(nn.Module):
    def __init__(self, config, features_count, step):
        ...
        self.shift = torch.tensor(list(product(
            (-1, 0, 1), repeat=3)))[None].float() * torch.tensor([[[
                2**(config.steps + 1 - step) /
                (W), 2**(config.steps + 1 - step) /
                (H), 2**(config.steps + 1 - step) / (D)
            ]]])[None]
        self.shift = self.shift.cuda()
    ...
    def forward(self, voxel_features, vertices):
        ...
        features_diff_from_center = features - features[:, :, :,
                                                        13][:, :, :,
                                                            None]  # 13 is the index of the center cordinate in shifts
        features_diff_from_center = features_diff_from_center.permute(
            [0, 3, 2, 1])
        features_diff_from_center = self.feature_diff_1(
            features_diff_from_center)
        features_diff_from_center = self.feature_diff_2(
            features_diff_from_center)
        features_diff_from_center = features_diff_from_center.permute(
            [0, 3, 2, 1])

        features_diff_from_center = self.sum_neighbourhood(
            features_diff_from_center)[:, :, :, 0].transpose(2, 1)
...
class LearntNeighbourhoodSampling(nn.Module):
    def __init__(self, config, features_count, step):
        ...
        # torch.nn.init.kaiming_normal_(self.sum_neighbourhood.weight, nonlinearity='relu')
        # torch.nn.init.constant_(self.sum_neighbourhood.bias, 0)
        self.shift_delta = nn.Conv1d(features_count,
                                     27 * 3,
                                     kernel_size=(1),
                                     padding=0).cuda()
    ...
    def forward(self, voxel_features, vertices):
        ...
        shift_delta = self.shift_delta(features).permute(0, 2, 1).view(
            B, N, 27, 1, 3)
        shift_delta[:, :,
                    0, :, :] = shift_delta[:, :,
                                           0, :, :] * 0  # setting first shift to zero so it samples at the exact point
        ...
        features = torch.cat(
            [features,
             neighbourhood.permute(0, 4, 1, 2, 3)[:, :, :, :, 0]],
            dim=1)
