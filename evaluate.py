...
def write_to_wandb(writer, epoch, split, performences, num_classes):
    ...
    for key, value in performences[split].items():
        log_vals[split + '_' + key + '/mean'] = np.mean(
            performences[split][key])
        for i in range(1, num_classes):
            log_vals[split + '_' + key + '/class_' + str(i)] = np.mean(
                performences[split][key][:, i - 1])
    try:
        wandb.log(log_vals)
    except:
        print('')
    ...
    def predict(self, data, config):
        ...
        if name == 'unet':
            ...
            for c in range(self.config.num_classes - 1):
                ...
                pred_voxels_rasterized = rasterizer(pred_vertices,
                                                    pred_faces).long()
    ...
    def save_results(self, predictions, epoch, performence, save_path, mode):
        ...
        for i, data in enumerate(predictions):
            ...
            if y_hat.points is not None:
                for p, (true_points,
                        pred_points) in enumerate(zip(y.points, y_hat.points)):
                    save_to_obj(
                        save_path + '/points/' + mode + 'true_' + str(i) +
                        '_part_' + str(p) + '.obj', true_points, [])
                    if pred_points.shape[1] > 0:
                        save_to_obj(
                            save_path + '/points/' + mode + 'pred_' + str(i) +
                            '_part_' + str(p) + '.obj', pred_points, [])

            if y_hat.mesh is not None:
                for p, (true_mesh,
                        pred_mesh) in enumerate(zip(y.mesh, y_hat.mesh)):
                    save_to_obj(
                        save_path + '/mesh/' + mode + 'true_' + str(i) +
                        '_part_' + str(p) + '.obj', true_mesh['vertices'],
                        true_mesh['faces'], true_mesh['normals'])
                    save_to_obj(
                        save_path + '/mesh/' + mode + 'pred_' + str(i) +
                        '_part_' + str(p) + '.obj', pred_mesh['vertices'],
                        pred_mesh['faces'], pred_mesh['normals'])
        ...
        if performence is not None:
            for key, value in performence.items():
                ...
                summary = ('{}: ' + ', '.join(
                    ['{:.8f}'
                     for _ in range(self.config.num_classes - 1)])).format(
                         epoch, *performence_mean)
                append_line(save_path + mode + 'summary' + key + '.txt',
                            summary)
                print(('{} {}: ' + ', '.join(
                    ['{:.8f}'
                     for _ in range(self.config.num_classes - 1)])).format(
                         epoch, key, *performence_mean))

                all_results = [('{}: ' + ', '.join(
                    ['{:.8f}'
                     for _ in range(self.config.num_classes - 1)])).format(*(
                         (i + 1, ) + tuple(vals)))
                               for i, vals in enumerate(performence[key])]
                write_lines(save_path + mode + 'all_results_' + key + '.txt',
                            all_results)
        ...
        if y_hat.voxel is not None:
            ...
            y_hats_voxels = F.upsample(y_hats_voxels[None, None].float(),
                                       size=xs.shape)[0, 0].long()
            ys_voxels = F.upsample(ys_voxels[None, None].float(),
                                   size=xs.shape)[0, 0].long()
            ...
            overlay_y_hat = blend_cpu(xs, y_hats_voxels,
                                      self.config.num_classes)
